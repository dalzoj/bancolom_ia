...
class CohereHandler(EmbeddingInterface):
    ...
    def embed_batch(self, data):
        try:
            ...
            print(
                f"ERROR: No se pudo generar el batch de embeddings — {e}",
                file=sys.stderr,
            )
            return []

    def embed_query(self, query):
        print("INFO: Generando embeddings.", file=sys.stderr)
        try:
            response = self._client.embed(
                texts=[query],
                model=self._model,
            ...
            print(
                f"ERROR: No se pudo generar el embedding de la consulta — {e}",
                file=sys.stderr,
            )
            return []
